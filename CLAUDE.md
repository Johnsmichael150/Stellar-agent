...
| 2026-07-24 | Bug sweep: issues #356, #357, #358, #359 | ✅ | #356: seller-agent.ts no longer crashes on unset/garbage `JOB_ID` — left `undefined` when unset (preserving the existing optional Step 3 submit path), throws a clear error only for non-numeric values. #359: found and fixed the real root cause — `landing/app.js` declared `const navLinks` twice in the same scope (nav-link tracking vs. the `#nav-links` container), a genuine `SyntaxError` confirmed via `node --check` that silently broke **every** feature in the script (scroll reveal, active nav, copy-to-clipboard, smooth scroll, mobile hamburger). Renamed the container ref to `navLinksContainer`. #357 and #358 turned out to be **already resolved** in current code — both real icon buttons (GitHub, Docs; there's no third "X" button) already have correct `aria-label`s, and the copy-to-clipboard toast handler already existed in app.js with matching CSS. They only appeared broken because of the #359 SyntaxError killing the whole script — no code change needed for either once #359 was fixed. |
...
- Plain `.js` files (like `landing/app.js`) aren't type-checked by anything in this repo — a `SyntaxError` (e.g. `const` redeclared in the same scope) can sit undetected and silently kill the *entire* script at parse time in the browser, taking down every feature it defines, not just the one near the bug. Run `node --check <file>.js` as a cheap sanity check any time you touch a non-TS script, since nothing else in CI catches this.
