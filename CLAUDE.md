...
| 2026-07-24 | Issues #360–#363: landing CTA/responsive + contract test coverage | ✅ | #360: dashboard CTA hrefs now `/app` + `data-dashboard-link`, driven by a `<meta name="marc-dashboard-url">` tag app.js rewrites on load (was hardcoded to an unrelated external domain). #361: added `@media (max-width: 480px)` for nav/hero/contract-card padding on small phones (768px breakpoint already existed). #362: added real event-payload assertions (`env.events().all().filter_by_contract(...)` vs `event.to_xdr(...)`) for `Registered`, `JobCreated`, `JobSubmitted`, `JobCompleted`, `JobCancelled`. #363: `create_job()` had no init-guard at all — it silently succeeded before `init()`; added a `not initialized` panic guard plus a `should_panic` test. **Discovered but did NOT fix (user's call):** `agent-identity` fails to compile on `main` as of `cd899fb` (soroban-sdk 26.1.0→27.0.2 dependabot bump) — see Gotchas. |
| 2026-07-29 | docs: add CONTRIBUTING.md | ✅ | Created comprehensive contributor guide covering (1) conventional commit format with scopes and examples, (2) TDD workflow with code examples, (3) test execution for Rust (`cargo test`) and TypeScript (`npm test --workspace`), (4) CLAUDE.md update protocol with mandatory post-task documentation, (5) self-audit checklist. All sections validated against current toolchain and project conventions in CLAUDE.md. GitHub CLI / PR workflow included. |
| 2026-07-29 | infra: add landing dev server | ✅ | Created `landing/package.json` + `landing/server.js`. Enables `npm run dev` preview on http://localhost:3001. Server serves static assets (index.html, style.css, app.js, robots.txt, og-image.svg, sitemap.xml) + emulates the Vercel `/api/contract-addresses` endpoint locally. SPA fallback to index.html for client-side routing. CORS headers and MIME types configured. Tested: npm install clean, server starts, GET / returns HTML, GET /api/contract-addresses returns JSON. No dependencies added (uses Node.js built-in http). |
| 2026-07-29 | feat(landing): add consent banner for external resources | ✅ | Created `consent-banner.js` (136 lines) + added consent banner HTML + CSS to landing page. Manages user consent for Google Fonts (may set cookies, esp. in GDPR regions) and analytics (Umami placeholder). Consent persisted in localStorage for 365 days. External font link deferred from `<head>` — only loaded after user accepts. Fallback system fonts (`-apple-system, BlinkMacSystemFont, system-ui, sans-serif`) used until consent given or fonts load. Banner styled responsive (desktop/tablet/mobile). Tested: markup present, consent-banner.js loads, banner visible on first visit. Ready for analytics integration (placeholder `loadAnalytics()` function included). |
| 2026-07-29 | feat(landing): add branded 404 error page | ✅ | Created `404.html` (123 lines) + `404.js` (60 lines) + updated `vercel.json` + modified `server.js`. Replaces Vercel's generic 404 with branded page featuring large "404" code with float animation, descriptive error message, CTA buttons (Return Home, Get Started, Launch Dashboard), and decorative geodesic background (lighter opacity). Nav + footer match landing. Same consent banner. Tested: localhost dev server returns 404.html with 404 status for nonexistent paths, HTML markup renders correctly, buttons link to home/sections/dashboard. Vercel config includes 404.html build + `errorPages` handler. |
| 2026-07-29 | git: push feat/landing-improvements to origin | ✅ | Created branch `feat/landing-improvements` from main (6a14626). Staged 11 files: CONTRIBUTING.md, CLAUDE.md updates, 6 landing page files (404.html, 404.js, consent-banner.js, package.json, server.js, vercel.json), 2 landing HTML/CSS modifcations. Commit 6ca6357 with conventional message covering dev server, consent banner, 404 page, and CLAUDE.md updates. Pushed to origin with `-u` flag (tracking set). Ready for pull request. |
...
- **`agent-identity` does not compile on `main` as of `cd899fb`** (soroban-sdk 26.1.0→27.0.2 bump, `e0c3d3d`, version-only diff). soroban-sdk 27.x removed `Vec` and `panic_with_error!` from the auto-glob — `contracts/agent-identity/src/lib.rs` needs `use soroban_sdk::{..., Vec, panic_with_error};`. Separately (not a 27.x issue), `get_agent()` returns `Agent` directly but `contracts/agent-identity/src/test.rs` calls `.unwrap()` on it in two places as if it returned `Option<Agent>`. `agentic-commerce` is unaffected (compiles and tests pass clean on 27.0.2). Left unfixed per explicit instruction when found during issues #360–#363 — needs its own PR.
- soroban-sdk 27.x: `#[should_panic(expected = "...")]` tests that panic via `panic_with_error!`/`contracterror` no longer put the variant name in the panic message — it renders as `HostError: Error(Contract, #N)` instead of containing e.g. `"AgentNotFound"`. Tests asserting on the string form of a `contracterror` variant (not a plain `panic!("string")`) will fail under 27.x even once the crate compiles. Discovered while temporarily patching agent-identity locally to verify a new test; not fixed (out of scope, see above).
- `env.events().all()` (testutils, 27.x) is scoped to the **last contract invocation only** — it does NOT accumulate across a whole test. Any client call after the one that published the event (another entry point, a read-only call, even `token.balance()`) resets it to empty. Assert events immediately after the action that publishes them, before making further calls.
- To assert an exact `#[contractevent]` payload in tests: `use soroban_sdk::{testutils::Events as _, Event};` then `assert_eq!(env.events().all().filter_by_contract(&contract_id), [my_event.to_xdr(&env, &contract_id)]);`. Use an array literal (`[...]`), not `std::vec![...]` — the contract crates are `#![no_std]` so bare `std::` paths don't resolve even under `#[cfg(test)]`; `ContractEvents` implements `PartialEq` against a plain array directly.
